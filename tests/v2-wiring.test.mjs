...
test('生产入口保留 V2 并独立装配 V3 地基，面板四 Tab 收敛且无 V1 integration API', async () => {
  ...
  for (const factory of ['createHostAdapter', 'createFoundationStore', 'createFoundationRuntime']) assert.equal((entry.match(new RegExp(`${factory}\\s*\\(`, 'g')) || []).length, 1, factory);
  ...
  assert.deepEqual([...html.matchAll(/data-tab="([^"]+)">([^<]+)/g)].map(match => [match[1], match[2]]), [['people', '千人'], ['events', '千结'], ['bonds', '双丝网'], ['next', '下一步']]);
});
