...
test('生产入口只装配 V2，面板四 Tab 收敛且无 V1 integration API', async () => {
  const [entry, panel, html, bootstrap] = await Promise.all([
  ...
  for (const factory of ['createArchiveV2Session', 'createArchiveV2Lifecycle', 'createArchiveV2Composition', 'createArchiveV2MemoryComposition', 'createArchiveV2FollowedProfileComposition', 'createArchiveV2DossierComposition']) {
