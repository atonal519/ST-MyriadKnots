...
test('真实面板入口按千人/千结/双丝网/设置映射视图，并恢复各页滚动位置', async () => {
  ...
  for (const value of ['教程文档', 'API 接口', '接口文档']) assert.ok(documentationCopy.includes(value));
  ...
  assert.deepEqual(documentRows.map(node => node.children.map(child => child.textContent)), [['教程文档', '教程文档'], ['API 接口', '接口文档']]);
  ...
  dialogActive = false;
  const copyExample = flatten(documentation).find(node => node.textContent === '接口文档');
  ...
  assert.equal(memoryControls.find(node => node.className === 'settings-hint'), undefined, '不展示固定解释，仍保留设置结果反馈');
