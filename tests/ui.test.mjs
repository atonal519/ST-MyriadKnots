...
test('构建产物可隔离加载且正式入口保留宿主桥接', async () => {
  ...
  assert.match(source, /\.\/src\/bootstrap\.js/); assert.match(manifest.js, /^dist\/qqj-app\.js\?v=/);
  const production = await readFile(new URL(`../${manifest.js.split('?')[0]}`, import.meta.url), 'utf8');
  assert.match(production, /\/scripts\/personas\.js/); assert.doesNotMatch(production, /(?:from\s*|import\s*\()\s*["']\.\.?(?:\/)/);
  assert.doesNotMatch(await readFile(new URL('../dist/index.js', import.meta.url), 'utf8'), /personas\.js|\.\.\/\.\.\//);
});

test('正式 SVG、dialog 语义和 manifest 均在生产源中', async () => {
  ...
  const manifest = JSON.parse(await readFile(new URL('../manifest.json', import.meta.url), 'utf8'));
  assert.match(fab, /viewBox="0 0 64 64"/); assert.match(fab, /currentColor/); assert.match(panel, /role="dialog"/); assert.match(panel, /aria-modal="true"/); assert.doesNotMatch(panel, /source-badge|<footer/); assert.doesNotMatch(manifest.description, /无视觉 UI/);
});

test('生产构建包含桌面/手机窗口壳与独立正文滚动约束', async () => {
  const dist = await readFile(new URL('../dist/index.js', import.meta.url), 'utf8');
  assert.match(dist, /width:360px/); assert.match(dist, /top:80px/); assert.match(dist, /right:20px/); assert.match(dist, /height:min\(600px,85dvh\)/); assert.match(dist, /max-height:85dvh/); assert.match(dist, /z-index:4000/);
...
test('手机窗口纯布局 seam 在320/390宽视口左右各10px，短屏仍保留 body 滚动', async () => {
  ...
  const dist = await readFile(new URL('../dist/index.js', import.meta.url), 'utf8'); assert.match(dist, /grid-template-rows:auto auto minmax\(0,1fr\) 24px/); assert.match(dist, /\.body\{min-height:0/); assert.match(dist, /scrollbar-gutter:stable/); assert.match(dist, /@media\(max-width:640px\)[\s\S]*grid-template-rows:auto auto minmax\(0,1fr\)/);
});
...
test('桌面几何覆盖 1920、1366 与 641，损坏和越界偏好安全回退或 clamp', () => {
  assert.deepEqual(desktopPanelSize(1920, 1080), { width: 360, height: 600, minWidth: 320, minHeight: 300, maxWidth: 1900, maxHeight: 1060 });
  assert.deepEqual(desktopPanelSize(1366, 768), { width: 360, height: 600, minWidth: 320, minHeight: 300, maxWidth: 1346, maxHeight: 748 });
  assert.deepEqual(desktopPanelSize(641, 700), { width: 360, height: 595, minWidth: 320, minHeight: 300, maxWidth: 621, maxHeight: 680 });
  assert.deepEqual(desktopPanelSize(1366, 768, { width: -1, height: '坏值' }).width, 360);
  assert.deepEqual(desktopPanelSize(1366, 768, { width: 9999, height: 9999 }).width, 1346);
  assert.deepEqual(desktopPanelPosition(1366, 768, 360, 600, { left: -900, top: 9999 }), { left: 10, top: 158 });
  const corrupt = geometryHarness({ saved: { 'qqj-panel-size': JSON.stringify({ width: 720, height: 700 }), 'qqj-panel-size-v2': '{坏 JSON', 'qqj-panel-pos-v2': '{坏 JSON' } }); assert.equal(corrupt.panel.style.width, '360px'); assert.equal(corrupt.panel.style.top, '80px');
});

test('桌面拖动仅主键且排除控件，超过 5px 才拖；cancel/lost capture 清理且位置不越界', () => {
  const harness = geometryHarness(), { dragHandle, panel, store } = harness;
  dragHandle.fire('pointerdown', { button: 2, clientX: 100, clientY: 100 }); dragHandle.fire('pointermove', { clientX: 300, clientY: 300 }); dragHandle.fire('pointerup'); assert.equal(store.has('qqj-panel-pos-v2'), false);
  const control = { closest: selector => selector.includes('button') ? {} : null }; dragHandle.fire('pointerdown', { target: control, clientX: 100, clientY: 100 }); dragHandle.fire('pointermove', { clientX: 300, clientY: 300 }); dragHandle.fire('pointerup'); assert.equal(store.has('qqj-panel-pos-v2'), false);
  dragHandle.fire('pointerdown', { clientX: 100, clientY: 100 }); dragHandle.fire('pointermove', { clientX: 105, clientY: 100 }); dragHandle.fire('pointerup'); assert.equal(store.has('qqj-panel-pos-v2'), false); assert.equal(panel.style.right, '20px');
  ...
  assert.deepEqual(JSON.parse(store.get('qqj-panel-pos-v2')), { left: 10, top: 10 }); assert.equal(panel.style.willChange, '');
  const saved = store.get('qqj-panel-pos-v2'); dragHandle.fire('pointerdown', { clientX: 10, clientY: 10 }); dragHandle.fire('pointermove', { clientX: 100, clientY: 100 }); dragHandle.fire('pointercancel'); assert.equal(store.get('qqj-panel-pos-v2'), saved); assert.equal(panel.classList.contains('is-gesturing'), false);
  dragHandle.fire('pointerdown', { clientX: 10, clientY: 10 }); dragHandle.fire('pointermove', { clientX: 100, clientY: 100 }); dragHandle.fire('lostpointercapture'); assert.equal(store.get('qqj-panel-pos-v2'), saved);
  dragHandle.fire('pointerdown', { clientX: 10, clientY: 10 }); dragHandle.fire('pointermove', { clientX: 100, clientY: 100 }); harness.viewport.fire('resize'); assert.equal(panel.classList.contains('is-gesturing'), false); assert.equal(store.get('qqj-panel-pos-v2'), saved);
});

test('桌面拉伸按视口和 320×300 clamp，只写设备偏好；临时缩屏与手机切换不污染保存值', () => {
  const harness = geometryHarness({ saved: { 'qqj-panel-size-v2': JSON.stringify({ width: 900, height: 700 }), 'qqj-panel-pos-v2': '{坏 JSON' } });
  ...
  assert.equal(panel.style.width, '320px'); assert.equal(panel.style.height, '300px'); assert.deepEqual(JSON.parse(store.get('qqj-panel-size-v2')), { width: 320, height: 300 });
  resizeHandle.fire('pointerdown', { clientX: 0, clientY: 0 }); resizeHandle.fire('pointermove', { clientX: 5000, clientY: 5000 }); resizeHandle.fire('pointerup', { clientX: 5000, clientY: 5000 });
  assert.equal(panel.style.width, '910px'); assert.equal(panel.style.height, '990px'); assert.deepEqual(JSON.parse(store.get('qqj-panel-size-v2')), { width: 910, height: 990 });
  store.set('qqj-panel-size-v2', JSON.stringify({ width: 900, height: 700 })); viewport.innerWidth = 800; viewport.innerHeight = 600; viewport.fire('resize');
  assert.equal(panel.style.width, '780px'); assert.equal(panel.style.height, '580px'); assert.deepEqual(JSON.parse(store.get('qqj-panel-size-v2')), { width: 900, height: 700 });
  ...
  resizeHandle.fire('pointerdown', { clientX: 0, clientY: 0 }); resizeHandle.fire('pointermove', { clientX: 100, clientY: 100 }); resizeHandle.fire('pointerup'); assert.deepEqual(JSON.parse(store.get('qqj-panel-size-v2')), { width: 900, height: 700 });
...
test('真实 dist bootstrap 生命周期：默认隐藏、唯一打开、关闭/Escape恢复焦点且 setState 不自动打开', async () => {
  const mod = await import('../dist/index.js?lifecycle=1'); const hosts = new Map(); const events = {}; const panelStates = []; let panelShow = 0; let panelClose = 0; let focusCount = 0; let formalReads = 0; let fabOpen; let wandOpen;
  ...
  const panelFactory = ({ onClose }) => { const host = { id: 'qqj-panel-host', hidden: true, style: {}, __qqjInstance: null }; let trigger; const panel = { host, show: source => { assert.notEqual(panelStates.at(-1)?.status, 'loading'); trigger = source; panelShow += 1; host.hidden = false; }, close: () => { panelClose += 1; host.hidden = true; onClose(); trigger?.focus?.(); }, setState(value) { panelStates.push(value); } }; host.__qqjInstance = panel; return panel; };
...
test('真实 dist UI：识别失败警告色保留旧列表与操作，成功清错，部分归一化显示有限提示', async () => {
  ...
  try {
    ...
    assert.equal(root.nodes['.status-label'].textContent, '人物档案尚未就绪'); assert.match(root.nodes['.status-dot'].className, /warn/);
