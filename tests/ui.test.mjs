...
test('正式 SVG、dialog 语义和 manifest 均在生产源中', async () => {
  ...
  assert.match(fab, /viewBox="0 0 64 64"/); assert.match(fab, /currentColor/); assert.match(panel, /role="dialog"/); assert.match(panel, /aria-modal="true"/); assert.doesNotMatch(panel, /source-badge/); assert.doesNotMatch(manifest.description, /无视觉 UI/);
});

test('生产构建包含桌面/手机窗口壳与独立正文滚动约束', async () => {
  const dist = await readFile(new URL('../dist/index.js', import.meta.url), 'utf8');
  assert.match(dist, /width:360px/); assert.match(dist, /top:80px/); assert.match(dist, /right:20px/); assert.match(dist, /height:min\(600px,85dvh\)/); assert.match(dist, /max-height:85dvh/);
...
test('手机窗口纯布局 seam 在320/390宽视口左右各10px，短屏仍保留 body 滚动', async () => {
  ...
  const dist = await readFile(new URL('../dist/index.js', import.meta.url), 'utf8'); assert.match(dist, /grid-template-rows:auto auto minmax\(0,1fr\) 24px/); assert.match(dist, /\.body\{min-height:0/); assert.match(dist, /scrollbar-gutter:stable/);
});
...
test('桌面几何覆盖 1920、1366 与 641，损坏和越界偏好安全回退或 clamp', () => {
  assert.deepEqual(desktopPanelSize(1920, 1080), { width: 360, height: 600, minWidth: 280, minHeight: 300, maxWidth: 1900, maxHeight: 1060 });
  assert.deepEqual(desktopPanelSize(1366, 768), { width: 360, height: 600, minWidth: 280, minHeight: 300, maxWidth: 1346, maxHeight: 748 });
  assert.deepEqual(desktopPanelSize(641, 700), { width: 360, height: 595, minWidth: 280, minHeight: 300, maxWidth: 621, maxHeight: 680 });
  ...
  const corrupt = geometryHarness({ saved: { 'qqj-panel-size-v2': '{坏 JSON', 'qqj-panel-pos-v2': '{坏 JSON' } }); assert.equal(corrupt.panel.style.width, '360px'); assert.equal(corrupt.panel.style.top, '80px');
});
...
test('桌面拖动仅主键且排除控件，超过 5px 才拖；cancel/lost capture 清理且位置不越界', () => {
  ...
  const control = { closest: () => ({}) }; dragHandle.fire('pointerdown', { target: control, clientX: 100, clientY: 100 }); dragHandle.fire('pointermove', { clientX: 300, clientY: 300 }); dragHandle.fire('pointerup'); assert.equal(store.has('qqj-panel-pos-v2'), false);
...
test('桌面拉伸按视口和 280×300 clamp，只写设备偏好；临时缩屏与手机切换不污染保存值', () => {
  ...
  assert.equal(panel.style.width, '280px'); assert.equal(panel.style.height, '300px'); assert.deepEqual(JSON.parse(store.get('qqj-panel-size-v2')), { width: 280, height: 300 });
  resizeHandle.fire('pointerdown', { clientX: 0, clientY: 0 }); resizeHandle.fire('pointermove', { clientX: 5000, clientY: 5000 }); resizeHandle.fire('pointerup', { clientX: 5000, clientY: 5000 });
