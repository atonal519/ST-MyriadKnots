...
function a(e, t, a = null) {
	let o = i(e, t), s = Math.max(0, o.width - 20), c = Math.max(0, o.height - 20), l = Math.min(280, s), u = Math.min(300, c), d = n(a?.width) && Number(a.width) > 0 ? Number(a.width) : 360, f = Math.min(600, Math.max(0, o.height * .85)), p = n(a?.height) && Number(a.height) > 0 ? Number(a.height) : f;
	return {
		width: r(d, l, s),
		height: r(p, u, c),
		minWidth: l,
...
function l({ panel: l, dragHandle: u, resizeHandle: d, storage: f = globalThis.localStorage, viewport: p = globalThis } = {}) {
	let m = null, h = null, g = null, _ = () => Number(p?.innerWidth) >= 641, v = () => i(p?.innerWidth, p?.innerHeight), y = (e, t) => {
		...
		let t = v(), n = Math.max(0, t.width - m.left - 10), i = Math.max(0, t.height - m.top - 10), a = Math.min(280, n), o = Math.min(300, i), s = r(m.width + e.x - m.startX, a, n), c = r(m.height + e.y - m.startY, o, i);
...
function d({ formal: e, people: t, settings: n, apiTools: r, loadState: i, initialRelations: a, reviewActions: o, onPluginEnabledChange: s, onClose: c } = {}) {
	...
	let p = f.querySelector(".view"), m = f.querySelector(".status-label"), h = f.querySelector(".status-meta"), g = f.querySelector(".status-dot"), _ = { status: "loading" }, v = null, y = !1, b = null, x = "people", S = "people", C = "", w = 0, T = 0, E = 0, D = null, O = !1, k = !1, A = null, j = !1, M = !1, N = null, P = /* @__PURE__ */ new Map(), F = /* @__PURE__ */ new Map(), I = null, L = null, R = !1, z = null, B = null, ee = () => [...f.querySelectorAll("button,input,select,textarea,[href],[tabindex]:not([tabindex=\"-1\"])")].filter((e) => !e.disabled && e.offsetParent !== null), V = () => {
		...
		let e = b;
		b = null, c?.(), e?.focus?.();
	}, U = (e) => Array.isArray(e) ? e.map(U) : !e || typeof e != "object" ? e : Object.fromEntries(Object.keys(e).sort().map((t) => [t, U(e[t])])), te = (e) => JSON.stringify(U(e)), ne = (e) => String(e?.chatId || e?.peopleFoundation?.state?.chatId || e?.people?.chatId || "unknown-chat"), re = (e, t) => [...e.filter((e) => e !== t), t], W = (e) => {
		...
		let t = ne(e), { profiles: n, profileMap: r } = W(e), i = new Set(n.map((e) => e.identityId)), a = P.get(t);
		if (a) {
			...
			for (let e of n) {
				let t = te(e), n = a.profileFingerprints.get(e.identityId);
				n !== void 0 && n !== t && (a.updatedOrder = re(a.updatedOrder, e.identityId), a.unreadUpdatedIds.add(e.identityId), a.railIds.includes(e.identityId) || a.railIds.push(e.identityId)), n === void 0 && !a.railIds.includes(e.identityId) && a.railIds.push(e.identityId), a.profileFingerprints.set(e.identityId, t);
			}
			if ((!a.selectedProfileId || !r.has(a.selectedProfileId)) && (a.selectedProfileId = n[0]?.identityId || null), a.selectedProfileId && !a.railIds.includes(a.selectedProfileId) && a.railIds.unshift(a.selectedProfileId), n.length <= 2) a.railIds = n.map((e) => e.identityId);
				...
				for (let e of ae(a, n)) if (a.railIds.includes(e) || a.railIds.push(e), a.railIds.length >= 2) break;
			}
			[
				"dossier",
				"more",
			...
			a = {
				...
				profileFingerprints: new Map(n.map((e) => [e.identityId, te(e)]))
			}, P.set(t, a);
		}
		return I = t, a;
	}, ce = (e) => ({
	...
	})[String(e?.code || "")] || "连接失败，请检查 API 配置后重试。", le = (e, t, n) => {
		...
		b && (b.disabled = !y), S && (S.disabled = !y), d?.addEventListener("change", async (e) => {
			...
			e && (e.value = "", e.placeholder = "输入 API Key"), K("保存后会清除 API Key。");
			...
			n.update({ apiPresetActiveId: t }), q({ preserveDrawers: !0 }), K(`已新增预设「${e}」。`, "success");
		}), p.querySelector("[data-action=\"preset-update\"]")?.addEventListener("click", () => {
			let e = p.querySelector("[data-setting=\"preset\"]")?.value, t = n.presets().find((t) => t.id === e);
			if (!t) return K("请先选择要更新的预设。", "error");
			n.upsertPreset(t.name, G(), e), q({ preserveDrawers: !0 }), K(`已更新预设「${t.name}」。`, "success");
		}), p.querySelector("[data-action=\"preset-rename\"]")?.addEventListener("click", () => {
			let e = p.querySelector("[data-setting=\"preset\"]")?.value, t = n.presets().find((t) => t.id === e);
			if (!t) return K("请先选择要改名的预设。", "error");
			let r = globalThis.prompt?.("新的预设名称", t.name)?.trim();
			r && (n.renamePreset(e, r), q({ preserveDrawers: !0 }), K(`已改名为「${r}」。`, "success"));
		}), p.querySelector("[data-action=\"preset-delete\"]")?.addEventListener("click", () => {
			let e = p.querySelector("[data-setting=\"preset\"]")?.value, t = n.presets().find((t) => t.id === e);
			if (!t) return K("请先选择要删除的预设。", "error");
			globalThis.confirm?.(`删除预设「${t.name}」？`) && (n.deletePreset(e), q({ preserveDrawers: !0 }), K("预设已删除。", "success"));
			...
			n.update({
				...
				apiStream: e.stream,
				apiPresetActiveId: p.querySelector("[data-setting=\"preset\"]")?.value || ""
			}), q({ preserveDrawers: !0 }), K("API 设置已保存。", "success");
			...
			let i = ue();
			e.currentTarget.disabled = !0, K("正在发送不含聊天与人物数据的短测试…");
			try {
				let e = await r?.testConnection?.(i);
				t === w && n.isEnabled() && K(`连接成功 · ${e?.model || "当前模型"}`, "success");
			} catch (e) {
				t === w && n.isEnabled() && K(ce(e), "error");
			} finally {
				t === w && n.isEnabled() && (e.currentTarget.disabled = !1);
			}
			...
			let i = ue();
			e.currentTarget.disabled = !0, K("正在读取模型列表…");
			try {
				let e = await r?.fetchModels?.(i), a = p.querySelector(".model-results");
				if (!a || t !== w || !n.isEnabled()) return;
		...
		let n = ie();
		n && (n.selectedProfileId = e, n.contentMode = "dossier", n.viewedOrder = re(n.viewedOrder, e), n.unreadUpdatedIds.delete(e), n.railIds.includes(e) || n.railIds.push(e), t && (z = {
			kind: "profile",
			id: e
		}), Ae(), Z(), Me());
	}, Pe = ({ availableWidth: e, itemWidths: t = {} } = {}, n = !0) => {
		let r = ie(), { profiles: i } = W(_), a = je();
		if (!r) return z = null, {
			changed: !1,
			railIds: []
		};
		...
		let o = Number(e), s = ae(r, i), c = oe(r, i);
		...
		let t = se(_), { selectedCharacters: n, selectedIds: r, profiles: i, profileMap: a } = W(_), o = new Map(n.map((e) => [e.identityId, e.displayName || "未命名人物"])), s = [...r], c = a.get(t?.selectedProfileId), l = new Map([[e.state?.personaId, "我"], ...i.map((e) => [e.identityId, o.get(e.identityId) || e.displayName || "未命名人物"])]), u = Y("div", "people-page"), d = ke(s, i.length > 0);
		d && u.append(d);
		let f = Y("div", "profile-rail-shell"), m = Y("div", "profile-switcher");
		m.setAttribute("role", "tablist"), m.setAttribute("aria-label", "切换人物档案");
		let h = oe(t, i).map((e) => a.get(e)).filter(Boolean);
		for (let e of h) {
		...
		if (!(["ready", "route_ready"].includes(e?.status) && e?.peopleFoundation?.status === "ready")) V(), z = null;
			...
			let t = ne(e), n = W(e).profileMap, r = ie();
